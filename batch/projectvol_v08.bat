...
if !ASK! == 1 (
    ...
    set /p LAN_CLAYNET="CLAYNET LAN (!LAN_CLAYNET!) ? "
    set /p LAN_PROJECT="PROJECT LAN (!LAN_PROJECT!) ? "
    set /p EXT="EXT (!EXT!) ? "
    set CONFIRM=1
    echo.

) else (
echo CLAYNET LAN = !LAN_CLAYNET!
echo PROJECT LAN = !LAN_PROJECT!
echo EXT = !EXT!
echo.
)
...
if !LAN! gtr 0 (
  echo | set /p="!LAN_MODE! SERVER: !IPADDR! (!CLAYNET_USE!!PROJECT_USE! )"
) 
echo.
echo ------------------------------------------------------------------------
echo.
